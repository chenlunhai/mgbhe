...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 无人店申请 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td>
					...
					<?php if($val['state'] < 1): ?><a href="javascript:;" onClick="handle_register(<?php echo ($val["id"]); ?>, 1)">拒绝</a>
					<a href="javascript:;" onClick="handle_register(<?php echo ($val["id"]); ?>)">通过</a><?php endif; ?>
				</td>
...
function handle_register(id, unset){
    layer.confirm('确定该操作吗？', {icon: 3, title:'提示'}, function(index){
        var icon = 2;
        $.ajax({
            url:'<?php echo U("Shop/handle_register");?>',
            type:'post',
            dataType:'json',
