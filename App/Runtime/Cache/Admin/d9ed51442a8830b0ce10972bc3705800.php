...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 厂家认证列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	...
	<div class="text-c" style="margin-top:10px;margin-bottom:10px;">
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td>
					<a style="text-decoration:none" class="ml-5" onclick="kuaidi('查看供应商资料','<?php echo U('Shop/supplier_register_show',array('id'=>$val['uid']));?>','900','300')" href="javascript:;" title="查看供应商资料">查看</a>
					<?php if(($val['supply_grade'] == 1) AND ($val['state'] == 1)): ?><a href="javascript:;" onClick="set_supplier(<?php echo ($val["uid"]); ?>, 0)">取消认证</a>
					<?php elseif($val['state'] == 0): ?>
						<a href="javascript:;" onClick="set_supplier(<?php echo ($val["uid"]); ?>, 1)">通过</a>
						<a href="javascript:;" onClick="set_supplier(<?php echo ($val["uid"]); ?>, 2)">拒绝</a><?php endif; ?>
				</td>
...
function set_supplier(id, state){
