    ...
    <div class="text-c" style="margin-top:10px;margin-bottom:10px;">
        <table cellpadding="3" cellspacing="0" class="table_98">
             <form  method="post" id="form1">
              ...
              <tr>
                <td>
                真实姓名：<input type="text" name="realname" value="<?php echo ($assign['realname']); ?>" style="width:100px" class="input-text" />
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td>
                    ...
                    <?php if($val['state'] < 1): ?><a href="javascript:;" onClick="handle_certification(<?php echo ($val["id"]); ?>, 1)">拒绝</a>
                    <a href="javascript:;" onClick="handle_certification(<?php echo ($val["id"]); ?>)">通过</a><?php endif; ?>
                </td>
...
function handle_certification(id, unset){
    layer.confirm('确定该操作吗？', {icon: 3, title:'提示'}, function(index){
        var icon = 2;
        $.ajax({
            url:'<?php echo U("Shop/handle_certification");?>',
            type:'post',
            dataType:'json',
