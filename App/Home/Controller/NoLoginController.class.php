...
class NoLoginController extends MainController {
        ...
        if(!$_POST){
            ...
            if(parent::$uid <1)
                ...
                if(!$grade){
                    ...
                    if($res['status']){
                        if(empty($res['data']))  $gourl = U('Index/authentication'); 
                        else{
                            if($res['data']['state']==2){  
                                #认证失败
                                $gourl = U('Index/authentication_fail');
                            }else if($res['data']['state']<2){
                                #认证成功
                                $gourl = U('Index/authentication_list');
                            }
                        }
                    }else{
                        $this->error($res['msg']);
