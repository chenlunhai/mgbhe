...
@foreach ($items as $item)
    @php
        ...
        if (Voyager::translatable($item)) {
            ...
            $slug = str_slug($item->translate('en')->title, '-');
        }
        ...
        if(!$item->children->isEmpty())
        {
            ...
            $linkAttributes = 'href="#' . $slug .'-dropdown-element" data-toggle="collapse" aria-expanded="'. (in_array('active', $listItemClass) ? 'true' : 'false').'"';
    ...
    <li class="{{ implode(" ", $listItemClass) }}">
        ...
        @if($hasChildren)
            <div id="{{ $slug }}-dropdown-element" class="panel-collapse collapse {{ (in_array('active', $listItemClass) ? 'in' : '') }}">
